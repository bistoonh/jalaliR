normalize_date <- function(dates, calendar = "jalali") {
  ...
  if(any(idx)) {
    valid_dates <- as.character(date_norm[idx])
    valid_dates <- sub("^([0-9]{4}-[0-9]{1,2})$", "\\1-01", valid_dates)
    parts <- do.call(rbind, strsplit(valid_dates, "-"))
    y <- parts[,1]
  ...
  # --- Validate dates using data.table if available ---
